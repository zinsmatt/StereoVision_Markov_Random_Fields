...
#define PROFILE(str, X) \
  { \
    ...
    float duration = static_cast<float>(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()) / 1000; \
    std::cout << str << " ... " << duration << " s" << std::endl;; \
  } \
...
//  cv::Mat image_r = cv::imread("../data/data_stereo/right/000002.png");
  ...
  int border_size = 16;
  cv::Mat disp;
  PROFILE("Standard stereo",
    disp = compute_stereo(image_l_grey, image_r_grey, disp_levels, block_size, StereoScoringMethod::SAD, border_size, false);
  )
  disp.convertTo(disp, CV_32F, 1.0 / disp_levels);

  cv::Mat disp2;
  PROFILE("Belief propagation",
    disp2 = stereo_belief_propagation(image_l_grey, image_r_grey, 20);
  )
  disp2.convertTo(disp2, CV_32F, 1.0 / LABELS);


  cv::namedWindow("Block Matching", cv::WINDOW_NORMAL);
  cv::imshow("Block Matching", disp);
  cv::namedWindow("Belief Propagation", cv::WINDOW_NORMAL);
  cv::imshow("Belief Propagation", disp2);
  cv::waitKey(0);
  ...
  disp2.convertTo(disp2, CV_8U, 255);
  cv::imwrite("disparity_belief.png", disp2);
